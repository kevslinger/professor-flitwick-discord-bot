...
async def log_command(
    ...
    Args:
        ...
        channel: discord.TextChannel, the channel the command was sent in
        author: discord.User, the discord user who invoked the command

    Sends a message to #bot-logs channel with information about the command.
    """
    ...
    if hasattr(channel, "name"):
        ...
        if channel.guild.id == discord_ids.DUELING_DISCORD_ID:
            embed = discord.Embed(
                author=author, description=f"Used `{command}` in {channel.mention}"
            )
