...
class PlayerOpsCog(commands.Cog, name="Player Operations"):
    ...
    async def join(self, ctx):
        ...
        if ctx.channel.id != discord_ids.LOBBY_CHANNEL_ID:
            ...
            await ctx.send(
                f"{ctx.author.mention} cannot join game from {ctx.channel.mention}. Head over "
        ...
        if len(current_player_role.members) >= player_cap:
            await ctx.message.add_reaction(EMOJIS[":x:"])
            await ctx.send(
                f"Sorry {ctx.author.mention}, we're already at full capacity for this week's LIVE game. "
        ...
        num_house_roles = 0
        num_tier_roles = 0
        # Count how many houses and how many tiers the player has selected.
        ...
        if num_house_roles == 1 and num_tier_roles == 1:
            if discord_ids.CURRENT_PLAYER_ROLE_ID not in [
                role.id for role in ctx.author.roles
            ]:
                await ctx.author.add_roles(current_player_role)
            # This reacts to the user's command with a confirmation checkmark
            await ctx.message.add_reaction(EMOJIS[":white_check_mark:"])
        # Error messages for having too few or too many houses/tiers
        else:
            await ctx.message.add_reaction(EMOJIS[":x:"])
            await ctx.send(
                f"{ctx.author.mention} Please make sure you have ONE house and ONE tier role before joining. "
    ...
    async def unjoin(self, ctx):
        ...
        Usage: `!unjoin`"""
        await logging_utils.log_command("join", ctx.channel, ctx.author)
        # Must use unjoin from the lobby channel
        if ctx.channel.id != discord_ids.LOBBY_CHANNEL_ID:
            await ctx.send(
                f"{ctx.author.mention} cannot unjoin game from {ctx.channel.mention}. Head over to "
            ...
            await ctx.message.add_reaction(EMOJIS[":x:"])
            return
        # Remove current player role
        await ctx.author.remove_roles(
            ctx.guild.get_role(discord_ids.CURRENT_PLAYER_ROLE_ID)
        )
        # React to command with confirmation checkmark
        await ctx.message.add_reaction(EMOJIS[":white_check_mark:"])

    @commands.command(name="msghost")
    # Only players with current player role may use msghost
    ...
    async def msghost(self, ctx):
        ...
        # Must use msghost in either gameplay channel or lobby channel.
        if (
            ctx.channel.id != discord_ids.GAMEPLAY_CHANNEL_ID
            and ctx.channel.id != discord_ids.LOBBY_CHANNEL_ID
        ):
            await ctx.send(
                f"{ctx.author.mention} cannot open host channel here. Please use `{ctx.prefix}msghost` in "
        ...
        try:
            # Create a new channel with the user and host
            _ = await ctx.guild.create_text_channel(
                f"{ctx.author.nick if ctx.author.nick is not None else ctx.author.name}",
            ...
            # If there is an error with creating the channel, send a message to the bot-logs channel.
        except discord.Forbidden:
            await ctx.guild.get_channel(discord_ids.BOT_LOG_CHANNEL_ID).send(
                f"ACCESS DENIED: Could not create channel "
