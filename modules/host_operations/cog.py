...
class HostOpsCog(commands.Cog, name="Host Operations"):
    ...
    def get_roster(self, guild, role):
        ...
        for player in current_players:
            roster_entry = [
                player.nick if player.nick is not None else player.name,
                "",
                "",
            ]
    ...
    async def openlobby(self, ctx):
        ...
        # Permissions of the lobby channel which we want to overwrite
        lock_perms = lobby_channel.overwrites_for(ctx.guild.default_role)
        # Enable people to send messages in lobby
        lock_perms.send_messages = True
        # Save the permissions to discord
        await lobby_channel.set_permissions(
            ctx.guild.default_role, overwrite=lock_perms
        )
        # Send a message in the lobby announcing the lobby is open
        await lobby_channel.send(
            f"It's {ctx.guild.get_role(discord_ids.TRIVIA_TUESDAY_ROLE_ID).mention},"
    ...
    async def startgame(self, ctx):
        """Locks `lobby`. Pings `CurrentPlayer` to announce the game is about to begin. Prints out the current roster of players
        ...
        # Permissions of the lobby channel which we want to overwrite
        lock_perms = lobby_channel.overwrites_for(ctx.guild.default_role)
        # Disallow people to send messages in lobby
        lock_perms.send_messages = False
        # Send a message announcing the game has begun
        embed = discord.Embed(
            title="LIVE Game has Begun!",
        ...
        roster = self.get_roster(
            ctx.guild, ctx.guild.get_role(discord_ids.CURRENT_PLAYER_ROLE_ID)
        )
        ...
        await gameplay_channel.send(
            f"{ctx.guild.get_role(discord_ids.CURRENT_PLAYER_ROLE_ID).mention} "
        ...
        # Send the roster to the game logs channel
    ...
    async def endgame(self, ctx):
        ...
        # Remove the current player role from everyone who has it.
        for player in current_player_role.members:
            await player.remove_roles(current_player_role)

        embed = discord.Embed(
            title=f"{datetime.now().strftime('%B %d, %Y')} Dueling LIVE Game Conclusion",
        ...
        # send announcement in both lobby and welcome channel
        await lobby_channel.send(embed=embed)
