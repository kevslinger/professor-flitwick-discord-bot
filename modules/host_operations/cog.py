...
class GameOpsCog(commands.Cog, name="Game Operations"):
    ...
    async def openlobby(self, ctx):
        ...
        await lobby_channel.send(f"It's {ctx.guild.get_role(host_operations_constants.TRIVIA_TUESDAY_ROLE_ID).mention},"
                                 ...
                                 f"{ctx.guild.get_channel(host_operations_constants.GAME_SIGNUPS_CHANNEL_ID).mention} "
                                 ...
                                 f"<:angryGeno:842965440619610124>")
    ...
    async def startgame(self, ctx):
        ...
        await gameplay_channel.send(f"{ctx.guild.get_role(host_operations_constants.CURRENT_PLAYER_ROLE_ID).mention} "
                                    f"The current game is set to begin!\n\nI should probably tell you how discord "
                                    f"dueling works but honestly I don't even know, so\n\n"
